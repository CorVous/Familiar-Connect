...
Round 3 finding:
  ...
  Root cause: G's three examples all show knowledge-gap deflection or stranger cold open.
  Haiku has no example of forming an in-stream opinion, so it pattern-matches
  "unexampled direct-address + nothing to deflect from" to silence.

Candidate H: G + fourth example showing Sapphire forming an opinion about Cor's
ongoing activity from observation — no knowledge gap, just a direct ask for her take.

Usage:
    cd /home/coder/workspace
    uv run python evals/haiku_reinforcements_r4.py
"""
...
REINFORCEMENTS: dict[str, str | None] = {
    "Haiku (baseline)": None,
    "Haiku + H (G + opinion example)": REINFORCEMENT_H,
}
...
def print_results(
    ...
    print("\n" + "=" * WIDTH)
    print("  SAPPHIRE REINFORCEMENT EVAL — ROUND 4 (candidate H)")
    print("=" * WIDTH)
