...
| D22 | **Vision wiring is validated across fields** (spec 02/08) by one classifier over `(slot, image_tools, describer, multimodal)` feeding both halves: on the `prose` slot, `image_tools` with neither `image_description_model` nor `multimodal` is a `ConfigError`; further incoherent combinations warn at the composition root. `config::IMAGE_TOOL_SLOT` names that gate and its diagnostics only — the text responder's client lookup and `tier_to_slot` keep their own `"prose"` literals, so retargeting the constant cannot silently repoint text chat. Blessing the multimodal-without-describer mode also required a neutral `NO_DESCRIPTION_PLACEHOLDER`: the old `"(no description model configured)"` rode beside the JPEG and into history, reading as "I cannot see images" while the model held one | Python accepted all of these silently — the rejected combination can hand back neither a description to persist nor an image in the payload, and vision capability is not inferable from a model name, so the remainder are reported rather than guessed. Scoped to `prose` because no other slot's flag reaches a registry; one classifier because the truth table had already drifted across three sites. Enforces what `tuning.md` § `image_tools` already documented | **BREAKING** (intended fix). No shipped profile is affected (`_default` leaves all three flags off), but a **deployed** `FAMILIARS_ROOT` config that set `[llm.prose].image_tools` with no delivery path booted before and now exits 1 — symptom and the two-line fix are in `getting-started/troubleshooting.md` |
