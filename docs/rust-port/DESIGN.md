...
| D22 | **Vision wiring is validated across fields** (spec 02/08) by one classifier over `(slot, image_tools, describer, multimodal)` feeding both halves: on the `prose` slot, `image_tools` with neither `image_description_model` nor `multimodal` is a `ConfigError`; five further incoherent combinations warn at the composition root. `config::IMAGE_TOOL_SLOT` replaces the `"prose"` literals in `commands::run` | Python accepted all of these silently — the rejected one can only ever return `"(no description model configured)"` after a fetch and a compression, and vision capability is not inferable from a model name, so the remainder are reported rather than guessed. Scoped to `prose` because no other slot's flag reaches a registry; one classifier because the truth table had already drifted across three sites. Enforces what `tuning.md` § `image_tools` already documented | **BREAKING** (intended fix; narrow — only the `prose` slot can be rejected, and no shipped profile is affected: `_default` leaves all three flags off) |
