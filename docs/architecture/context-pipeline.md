...
- **`familiar_connect.context.render`** — `assemble_chat_messages` owns the SillyTavern-accurate `Layer.depth_inject` placement (insert at position-from-end of the full chat list, clamped to after the system prompt). It also renders the Discord-syntax guide (how `<@id>` / `<#id>` / `https://discord.com/channels/...` tokens work) and the per-request **mention roster** — a `Participants you can mention:` block listing current-buffer Discord authors so the LLM can emit working `<@id>` tokens. The roster is carried on `ContextRequest.mention_roster` and built from the channel buffer authors in `bot.py`. Reply-rendered turns (`Alice (replying to Bob: "…"): text`) come through the normal recent-history path — `bot.py` writes the normalised, reply-prefixed content into `HistoryStore` so replays match live prompts.
