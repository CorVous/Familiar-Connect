...
- On each merged utterance, the bot calls `familiar.monitor.on_message(channel_id=voice_channel_id, author=..., text=..., is_lull_endpoint=True)` — the same entry point text messages use (see [Conversation flow](conversation-flow.md)). The `Author` comes from the voice-roster map captured when the bot joined the channel; a Deepgram diarization id maps to the `Author` for that speaker.
