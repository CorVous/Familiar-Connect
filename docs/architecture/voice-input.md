...
- Each finalised transcript is routed through `VoiceLullMonitor` (`src/familiar_connect/voice_lull.py`) for debouncing. The monitor uses audio-frame arrivals as the silence signal so `voice_lull_timeout` (default 5 s) reflects actual channel-wide quiet, not Deepgram's per-fragment endpointing.
