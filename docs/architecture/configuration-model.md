...
- **TTS voice** — `[tts]` section carries `voice_id` and `model` for the Cartesia client, `azure_voice` for Azure, and `greetings` (list of strings) for voice-channel join greetings.
