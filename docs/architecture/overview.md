...
5. The model calls `view_image(image_id="img_0")`. The handler fetches bytes, compresses to JPEG (1024 px longest edge, quality 85, 1 MB ceiling; iterates quality down by 5 until it fits), and calls the description model to get text. The prompt asks the model to describe appearance — colors, shapes, composition, text — and **not** name specific characters, people, franchises, brands, or other proper nouns. This keeps familiars from acquiring modern pop-culture knowledge (e.g. naming a specific Pokémon) that would break immersion.
...
**Configuration:**
- `[llm].image_description_model` — model name for vision-based description; empty = feature disabled. Descriptions are appearance-based (colors, shapes, composition, text), never naming specific entities or proper nouns.
