...
``finalize`` runs inference on buffered audio and emits single
``TranscriptionResult`` with ``is_final=True``.

Pair with ``[providers.turn_detection].strategy = "ten+smart_turn"`` —
Whisper has no built-in endpointer in this wiring (we don't lean on
its VAD; local turn detector decides when to fire ``finalize()``).

Numpy required at import time; ``faster_whisper`` itself loaded
...
class FasterWhisperTranscriber:
    ...
    def __init__(
        ...
        *,
        ...
        # WhisperModel handle — opaque ``Any`` so module imports
    ...
    def _load_model(self: Self) -> Any:  # noqa: ANN401 — opaque WhisperModel
        """Import faster_whisper, load CT2 model. Blocking."""
        try:
    ...
    def _transcribe(self: Self, audio: Any) -> str:  # noqa: ANN401 — np.ndarray
        ...
        # ``segments`` is a generator — must consume to drive model.
