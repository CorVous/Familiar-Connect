...
:class:`Event` — immutable, topic-addressed envelope. ``parent_event_ids``
carries session-lifetime lineage; derived SQLite rows carry
``source_turn_ids`` (forever).

:class:`TurnScope` — one per inbound turn. ``cancel()`` idempotent;
...
class Event:
    ...
    :param turn_id: scopes derived work; matches enclosing
        :class:`TurnScope`
    :param session_id: usually channel id
    :param parent_event_ids: lineage; empty for source events
    :param topic: routing key (see :mod:`familiar_connect.bus.topics`)
    :param sequence_number: monotonic per session; order-sensitive
        consumers sort or reject
    """
