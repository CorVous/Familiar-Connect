...
class TestTrailingReminder:
    ...
    @pytest.mark.asyncio
    async def test_trailing_system_renders_configured_timezone(
        self, tmp_path: Path
    ) -> None:
        captured: list[list[Message]] = []
        ...
        send = _CapturingSend()
        responder, _, _ = _make_responder(
            llm=llm, send=send, tmp_path=tmp_path, display_tz="America/Los_Angeles"
        )
        bus = InProcessEventBus()
        await bus.start()
        await responder.handle(_discord_text_event(content="hi"), bus)
        ...
        # LA abbreviation (PST/PDT by season), never UTC
        assert "PST" in content or "PDT" in content
        assert "It is now" in content
        assert " UTC" not in content

    @pytest.mark.asyncio
    async def test_trailing_system_carries_post_history_instructions(
        self, tmp_path: Path
    ) -> None:
        captured: list[list[Message]] = []
