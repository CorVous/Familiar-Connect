...
async def test_empty_when_no_reflections() -> None:
    ...
    layer = ReflectionLayer(store=AsyncHistoryStore(store), max_reflections=3)
...
async def test_max_reflections_caps_output() -> None:
    ...
    layer = ReflectionLayer(store=AsyncHistoryStore(store), max_reflections=2)
    out = await layer.build(_ctx())
...
async def test_max_reflections_zero_opts_out() -> None:
    ...
    layer = ReflectionLayer(store=AsyncHistoryStore(store), max_reflections=0)
    out = await layer.build(_ctx())
    assert not out
...
async def test_channel_scope_excludes_other_channels() -> None:
    ...
    layer = ReflectionLayer(store=AsyncHistoryStore(store), max_reflections=5)
...
async def test_max_tokens_truncates_block() -> None:
    ...
    layer = ReflectionLayer(
        store=AsyncHistoryStore(store), max_reflections=3, max_tokens=20
    )
    out = await layer.build(_ctx())
