//! Shared half-to-even ("banker's") rounding.
//!
//! Half-to-even rounds an exact `.5` to the nearest even integer.
//! Rust's [`f64::round`] rounds half away from zero, so the two disagree only at
//! an exact `.5`. Every site that needs half-to-even — the
//! `TierBudget::apply_curve` scaling (05/02), the voice-budget gap-ms recorder
//! (01), fact-importance paths — calls [`half_even`].
//!
