//! Parakeet-TDT buffer-and-finalize backend (subsystem 09; feature
//! `local-stt`).
//!
//! STUB — intentionally unimplemented. The intended backend runs NeMo's
...
//! sherpa-onnx behind the same buffer-and-finalize contract) has not been
//! chosen, so the `local-stt` feature carries no
