//! Seam conformance + policy set.
//!
//! Structural conformance is a compile-time trait bound: a type missing
//! `run`/`handle` simply fails to implement the trait and would not compile,
//! so there is no runtime rejection case to test.
...
// The seams store their identity (the `name` / `topics`
...
fn dummy_source_satisfies_trait() {
    ...
    // trait object proves it satisfies the seam.
...
fn all_policies_listed() {
    // Exactly these four policies exist.
